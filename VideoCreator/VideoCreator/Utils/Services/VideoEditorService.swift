...
final class VideoEditorService: NSObject {
    ...
    public func exportVideo(audio: AVURLAsset?, audioTimeRange: CMTimeRange?, completed: @escaping CompletedCombineBlock) {
        self.createDirectory()
        self.currentProgress = 0.0
        self.combineVideo { (success, url) in
            if success && url != nil {
                let video = AVURLAsset(url: url!)
                UISaveVideoAtPathToSavedPhotosAlbum(video.url.path, self, nil, nil)
                completed(true, url)
            } else {
                completed(false, nil)
            }
    ...
    fileprivate func makeMovementVideo(movement: ImageMovement, completed: CompletedCombineBlock?) {
        ...
        // path
        let path = Constants.LibraryURL.appendingPathComponent("\(self.movement).mov")
        print(path)
    ...
    fileprivate func makeTransitionVideo(transition: ImageTransition, completed: CompletedCombineBlock?) {
        ...
        // video path
        let path = Constants.LibraryURL.appendingPathComponent("\(transition).mov")
        print(path)
        self.deletePreviousTmpVideo(url: path)
    ...
    fileprivate  func createDirectory() {
        try? FileManager.default.createDirectory(at: Constants.LibraryURL, withIntermediateDirectories: true, attributes: nil)
    }
}
