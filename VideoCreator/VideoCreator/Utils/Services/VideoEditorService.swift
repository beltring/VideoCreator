...
class VideoEditorService {
    ...
    init(selectedPhotosArray: [UIImage]) {
        self.selectedPhotosArray = selectedPhotosArray
        self.photosArray = selectedPhotosArray
    }

    func buildVideoFromImageArray() {

        imageArrayToVideoURL = NSURL(fileURLWithPath: NSHomeDirectory() + "/VideoCreator/video.MP4")
        removeFileAtURLIfExists(url: imageArrayToVideoURL)

        guard let videoWriter = try? AVAssetWriter(outputURL: imageArrayToVideoURL as URL, fileType: .mp4) else {
...
//                            let aspectRatio = min(horizontalRatio, verticalRatio) // ScaleAspectFit
                            ...
                            print("MYLOG: Failed to allocate pixel buffer")
                            appendSucceeded = false
                        }
                    }
                    if !appendSucceeded {
                        print("MYLOG: !appendSucceeded")
                        break
                    }
                    frameCount += 1
                }
                ...
                videoWriter.finishWriting { () -> Void in
                    print("MYLOG: -----video1 url = \(self.imageArrayToVideoURL)")

                    self.asset = AVAsset(url: self.imageArrayToVideoURL as URL)
                    self.exportVideoWithAnimation()
//                    UISaveVideoAtPathToSavedPhotosAlbum(self.imageArrayToVideoURL.path!, self, nil, nil)
                }
            })
        }
    }

    private func removeFileAtURLIfExists(url: NSURL) {
        if let filePath = url.path {
            ...
            if fileManager.fileExists(atPath: filePath) {
                do{
                    ...
                    print("MYLOG: Couldn't remove existing destination file: \(error)")
                }
            }
        }
    }

    private func exportVideoWithAnimation() {
        ...
        let animatedVideoURL = NSURL(fileURLWithPath: NSHomeDirectory() + "/VideoCreator/video.MP4")
        ...
        assetExport.exportAsynchronously(completionHandler: {
            ...
            case  AVAssetExportSession.Status.failed:
                print("MYLOG: failed \(String(describing: assetExport.error))")
            case AVAssetExportSession.Status.cancelled:
                print("MYLOG: cancelled \(String(describing: assetExport.error))")
            default:
                UISaveVideoAtPathToSavedPhotosAlbum(animatedVideoURL.path!, self, nil, nil)
                print("MYLOG: Exported")
            }
        })
    }
}
